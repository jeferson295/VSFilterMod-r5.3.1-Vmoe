...
The current port contains a suspected SIMD-path `\ortho1` regression. Do not
change renderer code based only on compilation: compare the two ortho samples
with the computerfan reference, record the result, and review the minimal source
diff separately.
